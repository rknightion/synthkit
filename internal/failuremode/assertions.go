...
var assertions = []Assertion{
	...
	{Mode: "slow_query_storm", QueryAPI: "prometheus", Query: "max(aws_docdb_read_latency_average{dimension_DBClusterIdentifier=\"{{target}}\"})", Direction: DirectionIncrease, Source: "signals/cw.md [slug: cw-docdb]", Priority: 100},
	...
	{Mode: "error_spike", QueryAPI: "prometheus", Query: "sum(rate(traces_spanmetrics_calls_total{service_name=\"{{target}}\",status_code=\"STATUS_CODE_ERROR\"}[5m]))", Direction: DirectionIncrease, Source: "signals/apm.md [slug: apm-calls]", Priority: 80},
	{Mode: "latency_storm", QueryAPI: "prometheus", Query: "sum(rate(traces_spanmetrics_latency_sum{service_name=\"{{target}}\"}[5m])) / sum(rate(traces_spanmetrics_latency_count{service_name=\"{{target}}\"}[5m]))", Direction: DirectionIncrease, Source: "signals/apm.md [slug: apm-latency]", Priority: 70},
	...
	{Mode: "pod_crashloop", QueryAPI: "prometheus", Query: "sum(increase(kube_pod_container_status_restarts_total{cluster=\"{{target}}\"}[5m]))", Direction: DirectionIncrease, Source: "signals/k8s.md", Priority: 100},
	{Mode: "oom_kill", QueryAPI: "prometheus", Query: "sum(kube_pod_container_status_last_terminated_reason{cluster=\"{{target}}\",reason=\"OOMKilled\"})", Direction: DirectionIncrease, Source: "signals/k8s.md", Priority: 100},
	...
	{Mode: "web_vitals_degraded", QueryAPI: "loki", Query: "avg_over_time({service_name=\"{{target}}\",kind=\"measurement\"} |= \"type=web-vitals\" | logfmt | unwrap value_lcp [5m])", Direction: DirectionIncrease, Source: "signals/logs.md [slug: logs-faro]", Priority: 100},
}
