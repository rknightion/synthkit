...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 README uses port 8088, qualified blueprint/workload scaling IDs, and authenticated mutation examples when CONTROL_TOKEN is set
- [x] #2 README no longer implies an enabled YAML field and clearly distinguishes a synthetic workload model from deploying a real application
- [x] #3 Quick Start declares jq as optional or provides a dependency-free fallback
- [x] #4 Blueprint docs correctly describe bundled, selected-directory, custom paste/upload, and git-source paths without telling users to copy into an ignored BLUEPRINT_DATA_DIR root
- [x] #5 Control docs explain loaded, enabled, emitting, staged, pending, and active states and expose status, inventory, health, diagnostics, and pending entrypoints
- [x] #6 Blueprint and scenario array endpoints are documented as full replacements, not additive mutations
- [x] #7 Landing copy states the first Grafana-visible outcome before protocol/architecture detail
<!-- AC:END -->
