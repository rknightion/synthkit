...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 The report distinguishes no-construct-exists from construct-exists-but-unmapped, and the k8s-monitoring collector components are classified correctly
- [x] #2 Names that map to an existing construct are mapped, so the unmapped set shrinks to genuinely unmodelled products
- [x] #3 The skeleton never emits the same addon kind twice from one capture
- [x] #4 The report states what it detected and deliberately skipped, so silence is never ambiguous
- [x] #5 Verified by re-running against the lab capture: every line of the report is true of that cluster
<!-- AC:END -->
