...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 The form name and YAML name have one documented precedence or are required to match, and the response shows the effective namespaced runtime identity
- [x] #2 Save automatically validates the current YAML or remains disabled until that exact content has passed validation
- [x] #3 Editing YAML invalidates any prior validation result
- [x] #4 Validation checks the prospective loaded/staged set and reports identity or substrate collisions before restart
- [x] #5 The UI either provides a real YAML file chooser or consistently calls the feature Paste YAML while retaining the JSON automation API
- [x] #6 Empty state offers bundled example, paste YAML, and git-source routes plus apply instructions
<!-- AC:END -->
