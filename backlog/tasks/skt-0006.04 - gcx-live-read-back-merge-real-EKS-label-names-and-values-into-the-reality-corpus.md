...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 A documented make target reads real metric names, label names and label values for the in-scope families off an operator-selected live stack via gcx
- [x] #2 Read-back output merges into the corpus as entries tagged with the EKS substrate and the capture date
- [x] #3 The target fails with a clear message when no gcx context or credential is configured, and never proceeds against a default or guessed stack
- [x] #4 The scope covers EKS node/pod identity and core CloudWatch families including stat-suffix expansion, and explicitly excludes the AI/LLM areas
- [x] #5 No credential value, stack identifier, or account identifier is written into committed files, corpus entries, or documentation
- [x] #6 The findings report distinguishes drift that only the live read-back can see from drift the k3d lab already covers
<!-- AC:END -->
