...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 Standing deployments pin a released tag or digest by default and documentation explains when mutable main/latest are appropriate
- [x] #2 Upgrade records the current and candidate image digest, validates the candidate, and retains a concrete rollback target
- [x] #3 Rollback commands and post-rollback readiness/data checks are documented and tested
- [x] #4 CI or a release gate exercises docker-compose.yml with the published/version-matched image, writable state, healthcheck, and fake sink
- [x] #5 Release verification checks signature/provenance and reports the running synthkit version and image digest
<!-- AC:END -->
