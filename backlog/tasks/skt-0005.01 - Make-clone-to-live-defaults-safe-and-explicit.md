...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 A direct run created from .env.example binds the control plane to loopback unless the user explicitly opts into broader exposure
- [ ] #2 Docker Compose overrides only the container-internal HTTP bind so loopback-safe host publishing still works
- [ ] #3 Every primary live-start path explicitly transitions DRY_RUN to false and verifies /control/status reports dry_run=false
- [ ] #4 Invalid DRY_RUN boolean values fail with an actionable configuration error rather than silently remaining dry-run
- [ ] #5 Fresh-clone preflight and .env creation preserve mode 0600 without printing secrets
<!-- AC:END -->
