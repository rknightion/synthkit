...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 kubelet_pod_start_duration_seconds emits the captured kubelet bucket set
- [x] #2 coredns_proxy_request_duration_seconds emits the full captured bucket set including 4.096 and 8.192
- [x] #3 Every other histogram family the report flags for bucket bounds is checked, and either corrected or recorded as already matching
- [x] #4 The captured bucket sets are recorded in the relevant signals/ area file with provenance and date
- [x] #5 make signal-fidelity reports no remaining bucket_bound_mismatch contradiction
<!-- AC:END -->
