...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 Non-loopback startup requires an explicit trusted-network or TLS-proxy acknowledgement and warns/fails when CONTROL_TOKEN is absent
- [x] #2 Documentation states which GET and mutation surfaces are authenticated and never suggests sending Basic credentials over plaintext untrusted networks
- [x] #3 A decision is implemented and documented for authenticating sensitive read routes such as raw blueprint/config topology
- [x] #4 Setup creates .env mode 0600 and blueprint/state directories and staged files with least-privilege modes
- [x] #5 Runbook/security docs correctly state that blueprint source state persists token_env_var names, not resolved token values
- [x] #6 Security review covers bind, auth, read exposure, filesystem permissions, and backward compatibility
<!-- AC:END -->
