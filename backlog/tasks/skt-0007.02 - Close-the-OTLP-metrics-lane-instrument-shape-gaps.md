...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 The lane encodes every instrument shape the SKT-0007.01 in-scope set requires, including exponential histograms
- [x] #2 Temporality behaviour is evidenced against reality and documented, not assumed to match the promrw cumulative rule
- [x] #3 What the lane does or does not put on the wire for instrumentation scope is stated explicitly in signals/otlp-metrics.md
- [x] #4 Encoding remains hand-rolled with no OTel SDK dependency outside internal/selfobs
- [x] #5 Existing web_service OTLP output is unchanged, proven by its live-validated shape still matching
- [x] #6 Tests cover each instrument shape and the temporality decision
<!-- AC:END -->
