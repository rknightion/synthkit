...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 The chart deploys into k3d and its emissions are captured by the existing receiver
- [x] #2 Every lane the deployment declares is confirmed arriving, and a lane that silently disables itself is reported as a failure rather than an absence
- [x] #3 What arrives is compared against the committed inventory using the existing comparison, not a new one
- [x] #4 Counter behaviour across a pod restart is checked, since a reset reads as a spike on any dashboard
- [x] #5 The check runs in CI on a schedule, not only by hand
<!-- AC:END -->
