...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 A blueprint switch gates OTel-native cluster metrics; absent it, k8s_cluster emits exactly what it emits today
- [x] #2 Every emitted family and attribute is sourced from k8sclusterreceiver or kubeletstatsreceiver metadata with provenance and date
- [x] #3 No family name is derived from the existing kube_* or container_* Prometheus names
- [x] #4 SK-85 is resolved, or the kubeletstats families are not emitted
- [x] #5 signals/otlp-metrics.md covers every added family with provenance
- [x] #6 A blueprint modelled on k8s-minimal demonstrates the base surface emitting OTel-native end to end
- [x] #7 The construct imports no OTel SDK, selfobs or profiling; TestCatalogImportIsolation passes
<!-- AC:END -->
