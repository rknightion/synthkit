...
> This in-repo copy is **authoritative for synthkit** — an agent with only this checkout has the
> whole model, with no dependency on one machine's untracked home directory. It is a byte-for-byte
> copy below this header: do not summarise, compress or adapt it here. When the source changes, the
> edit is not finished until this copy is re-imported in the same change.
>
> Repo-specific rules are **not** here — they live in the **Wave operating model** doc.
