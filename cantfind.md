...
| SK-85 | `k8s_cluster` (OTel-native lane) | The `kubeletstatsreceiver` DEFAULT-ENABLED metric name set. `k8sclusterreceiver` names are confirmed from its `metadata.yaml`; kubeletstats is confirmed only for the opt-in utilisation ratios (`k8s.container.cpu.node.utilization`, `k8s.pod.cpu.node.utilization`, `k8s.container.memory.node.utilization`, `k8s.pod.memory.node.utilization`) and the `node`/`pod`/`container`/`volume` metric-group structure. The default container/pod/node CPU and memory family names are UNCONFIRMED. | NOT emitted — no OTLP metrics lane declared | ctx7 (`/open-telemetry/opentelemetry-collector-contrib`, `receiver/kubeletstatsreceiver/documentation.md`) · live-capture |
