...
| SK-87 | `envoy_gateway` | The metric names emitted on the Envoy Gateway OpenTelemetry stats sink, for both the `EnvoyProxy` data plane and the `EnvoyGateway` control plane. The sink config is confirmed; whether it ships the `envoy_*`/`xds_*`/`watchable_*`/`controller_runtime_*` names verbatim or an OTel-shaped name set is UNCONFIRMED. | NOT emitted — no OTLP metrics lane declared | vendor-doc (gateway.envoyproxy.io proxy-metric / gateway-observability) · ctx7 · live-capture |
