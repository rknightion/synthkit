...
| SK-86 | `beyla_agent` | The OTLP instrument names Beyla emits under `internal_metrics.exporter: otel`. The config switch is confirmed (`disabled`/`prometheus`/`otel`), but whether the OTLP form uses the dotted semconv spelling of the `beyla_internal_*`/`beyla_bpf_*` families or a different name set is UNCONFIRMED. Un-mangling the Prometheus spelling is NOT an acceptable derivation. | NOT emitted — no OTLP metrics lane declared | vendor-doc (`grafana/beyla` internal-metrics-reporter) · ctx7 · live-capture **2026-09-04 BOUNDARY SHARPENED — not capturable via k8s-monitoring.** chart 4.5.0's `feature-auto-instrumentation/templates/_beyla-config.tpl:17` builds `$internalMetrics := dict "prometheus" (dict "port" $targetPort)` and merges it as an OVERRIDE over user config, so `internal_metrics.prometheus.port` is injected unconditionally and no values file can remove it (nulling the block and the key were both tried and the port returns). Beyla exits with `wrong Beyla configuration: you can't enable both OTEL and Prometheus internal metrics`, so every otel-only route through this chart CRASHLOOPS the DaemonSet — verified the expensive way on the live lab, 2 of 4 pods down before revert. Resume needs a STANDALONE Beyla with a hand-written config (k3d), NOT the chart-managed one. | NOT emitted — no OTLP metrics lane declared | live-capture (standalone Beyla in k3d, hand-written config, NOT via k8s-monitoring) |
| SK-87 ⚠ PARTIAL (data plane RESOLVED 2026-09-04) | `envoy_gateway` | RESOLVED for the DATA PLANE: an OpenTelemetry stats sink was added to the live `EnvoyProxy` CR alongside its Prometheus one and captured on the wire (`e2e/lab/captures/envoy-gateway-otlp-fc9ca6cd2ec0eba6.md`, 162 names). It ships an **OTel-shaped set in dotted Envoy stat spelling** — `cluster.circuit_breakers.cx_open`, `http.*`, `listener.*`, `server.*`, `sds.*` — with **zero** `envoy_*`/`xds_*`/`watchable_*`/`controller_runtime_*` names, so it shares no name with the Prometheus scrape surface and neither spelling derives from the other. Instruments: 78 Sum / 69 Gauge / 15 Histogram. **Unit is EMPTY on all 162, InstrumentationScope is EMPTY, and resource attributes carry only `telemetry.sdk.*` — no `service.name`, no `k8s.*`.** Datapoint attributes are dotted `envoy.*` (cluster_name, http_conn_manager_prefix, listener_address, xds_resource_name, response_code, response_code_class, rds_route_config, worker_id, tls_certificate) plus bare `socket_match_name` and `priority`. STILL PENDING: the **EnvoyGateway CONTROL PLANE** (`watchable_*`, `controller_runtime_*`) — a separate component with its own telemetry config, not covered by `telemetry.metrics.sinks` on EnvoyProxy. | NOT emitted — no OTLP metrics lane declared | live-capture (add an OTel sink to the EnvoyGateway controller's own telemetry config and repeat) |
