...
| [`signals/runtime-node.md`](./signals/runtime-node.md) | Node.js prom-client default metrics | blueprint | promrw | `runtime-node` |
...
| [`signals/otlp-native-verdicts.md`](./signals/otlp-native-verdicts.md) | Per-kind verdicts (SKT-0007.01): which catalog kinds have a real OTel-native metric form, which are Prometheus-scrape-only and must never gain an OTLP lane, and which are unresolved. Gates entry to `otlp-metrics.md`; asserts no emitted family of its own | — | — | (no families) |
...
| [`signals/sigil.md`](./signals/sigil.md) | Grafana AI Observability / sigil — three concurrent lanes: native gRPC generation ingest (Lane A; content-bearing sanctioned exception), OTLP traces (Lane B; coding 2-level + general 3-scope span trees), and `gen_ai_client_*` + `agento11y_eval_*` metrics (Lane C); no Loki lane; `ai_agent` workload | blueprint | otlp + promrw | `sigil-lane-a`, `sigil-lane-b`, `sigil-spans-coding`, `sigil-spans-coding-tool`, `sigil-spans-general-envelope`, `sigil-spans-general-llm`, `sigil-lane-c`, `sigil-gen-ai-metrics`, `sigil-eval-metrics`, `sigil-archetypes`, `sigil-failure-modes`, `sigil-cardinality` |
